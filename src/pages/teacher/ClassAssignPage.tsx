...
export default function ClassAssignPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const [{ data: sts }, { data: existing }] = await Promise.all([
        ...
        supabase.from('class_assignments').select('student_id').eq('class_id', id!),
      ])
  ...
  const selectAll = () => {
    ...
    const allSelected = filtered.every((s) => assigned.has(s.id))
    if (allSelected) setAssigned((prev) => { const next = new Set(prev); allIds.forEach((id) => next.delete(id)); return next })
    else setAssigned((prev) => { const next = new Set(prev); allIds.forEach((id) => next.add(id)); return next })
  }

  const handleSave = async () => {
    setSaving(true)
    await supabase.from('class_assignments').delete().eq('class_id', id!)
    if (assigned.size > 0) {
      const rows = Array.from(assigned).map((student_id) => ({ class_id: id!, student_id }))
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <PageHeader title="Assign Students to Class" backTo="/teacher/classes" />
      <StudentFilter
      ...
      <div className="flex justify-between items-center mb-2 px-1">
        <span className="text-sm text-muted-foreground">{filtered.length} students shown</span>
        <button type="button" onClick={selectAll} className="text-xs text-[#6C63FF] hover:underline">
          {filtered.every((s) => assigned.has(s.id)) ? 'Deselect All' : 'Select All'}
        </button>
      </div>
      {loading ? (
      ...
      <div className="flex gap-3">
        <Button onClick={handleSave} disabled={saving} className="rounded-pill bg-[#6C63FF] hover:bg-[#5a52d5] flex-1">
          {saving ? 'Saving...' : `Save (${assigned.size} assigned)`}
        </Button>
        <Button variant="outline" onClick={() => navigate('/teacher/classes')} className="rounded-pill">Cancel</Button>
      </div>
    </div>
  )
}
