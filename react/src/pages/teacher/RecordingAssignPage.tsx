...
export default function RecordingAssignPage() {
  ...
  const handleSave = async () => {
    ...
    if (assigned.size > 0) {
      const rows = Array.from(assigned).map((student_id) => ({ recording_id: id!, student_id }))
      const { error } = await supabase.from('recording_assignments').insert(rows)
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <PageHeader title="Assign Recording to Students" backTo="/teacher/recordings" />
      ...
      <div className="flex gap-3">
        ...
        <Button variant="outline" onClick={() => navigate('/teacher/recordings')} className="rounded-pill">Cancel</Button>
      </div>
    </div>
  )
}
